...
**Update — visually confirmed on-screen, same session, shortly after:** the USB stdout-read problem above turned out not to matter. `pongoOS`'s console output was never only going over USB in the first place — `src/kernel/support/io.c`'s `_write()` calls both `serial_putc()` *and* `screen_putc()` for every byte, and `src/drivers/framebuffer/fb.c`'s `screen_init()` reads the framebuffer straight out of `gBootArgs->Video` — the same framebuffer iBoot already has live on the physical screen (confirmed first by seeing checkra1n's own boot logo render there). So the fix wasn't debugging USB at all: just **look at the phone screen**. Running `omerta` (via `issue_cmd.py`, write-only, no read needed) rendered the banner directly on-screen ("OMERTA iOS -- phase2 pongoOS boot environment" visible, confirmed by the person at the device). Running `bootx` afterward re-printed the banner via the `preboot_hook` chain (confirming step 7) and the boot continued through to iOS (`ideviceinfo` reachable again afterward, `ProductVersion: 12.5.8`).

**Phase 2 steps 6 and 7 (per the root `PLAN.md` staged list) are now genuinely confirmed, not just inferred.** The USB stdout-read issue above is still real and still unsolved, but it's no longer blocking — the framebuffer console is the actual, working confirmation channel for this phase.
